        ...
        const fetchResults = async () => {
            ...
            if (latitude && longitude && radius) {
                try {
                    ...
                    const { data } = await axios.get(`http://localhost:5000/restaurant/getWithInRadius?latitude=${latitude}&longitude=${longitude}&radius=${radius}&page=${currentPage}&limit=${limit}`)
            ...
            {!loading && (
                restaurants?.length > 0 ? (
                    <div className='flex flex-col pt-24'>
                        <div className='max-w-[2520px] mx-auto xl:px-20  md:px-10 sm:px-2 px-4 text-3xl font-bold'>Filtered Results</div>
                        <Container>
                            <div
                                className="pt-5 grid  grid-cols-1  sm:grid-cols-2  md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 gap-8"
                            >
