...
const Home = () => {
    ...
    useEffect(() => {
        const loadRestaurants = async () => {
            ...
            try {
                const { data } = await axios.get(`${server}/restaurant/get-all?page=${currentPage}&limit=${limit}`) 
                // const { data } = await axios.get(`http://localhost:5000/restaurant/get-all?page=${currentPage}&limit=${limit}`) 
                
                // Fetch with the current page and items per page
                if (data) {
